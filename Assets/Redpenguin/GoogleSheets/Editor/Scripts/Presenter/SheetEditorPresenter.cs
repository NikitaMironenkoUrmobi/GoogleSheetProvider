...
using Redpenguin.GoogleSheets.Editor.View;
...
namespace Redpenguin.GoogleSheets.Editor.Presenter
{
  public class SheetEditorPresenter
  {
      ...
      GoogleSheetProviderSettings settings)
    {
      ...
      SetView(view);
      ModelViewLink(model, settings.SerializationGroups);
      LoadMeta();
    }
    ...
    private void SetLoadState(ChangeEvent<bool> evt)
    {
      ...
      var groupData = _settings.groupsMetaData.Get(_spreadSheetSo.SheetDataTypeName);
    ...
    private void ModelViewLink(ScriptableObject scriptableObject, List<SerializationGroup> serializationGroups)
    {
      ...
      _containerLabel.text = _spreadSheetSo.SheetDataTypeName;
    }
